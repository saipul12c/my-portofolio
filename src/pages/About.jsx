...
>
  ...
  <div className="flex flex-wrap justify-center gap-4">
    {softSkills.skills.map((skill, i) => {
      const colorMap = {
        pink: { bg: "bg-pink-400/30", text: "text-white" },
        yellow: { bg: "bg-yellow-400/30", text: "text-white" },
        blue: { bg: "bg-blue-400/30", text: "text-white" },
        green: { bg: "bg-green-400/30", text: "text-white" },
        purple: { bg: "bg-purple-400/30", text: "text-white" },
        orange: { bg: "bg-orange-400/30", text: "text-white" },
        cyan: { bg: "bg-cyan-400/30", text: "text-white" },
        gray: { bg: "bg-gray-400/30", text: "text-white" }
      };
      const skillColor = colorMap[skill.color] || { bg: "bg-white/10", text: "text-white" };

      const levelColors = {
        Dasar: { bg: "bg-gray-700", text: "text-white" },
        Menengah: { bg: "bg-indigo-500", text: "text-white" },
        Mahir: { bg: "bg-green-600", text: "text-white" }
      };
      const levelColor = levelColors[skill.level] || { bg: "bg-white/20", text: "text-white" };

      return (
        <Link key={i} to={`/skills/${skill.id}`} className="relative group">
          <span
            className={`px-5 py-2 ${skillColor.bg} ${skillColor.text} border border-white/20 rounded-full text-sm font-semibold hover:scale-105 transition-all inline-flex items-center gap-2`}
          >
            {skill.name}
            {/* Badge Level */}
            <span
